...
if (!empty($_POST["btnregistrar"])) {
    ...
    if (empty($academia) || empty($profesor) || empty($dias) || empty($horario) || empty($nombre) || empty($apellido) || empty($numero_usuario) || empty($firma)) {
        ...
        $stmt = $mysqli->prepare("INSERT INTO `registrosInfantiles` (`academia`, `profesor`, `dias`, `horario`, `apartado`, `nombre`, `apellido`, `numero_usuario`, `email`, `paquete`, `precio`, `folio`,  `fecha_hora`, `reglamento`, `firma`) 
                                            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
        $stmt->bind_param("ssssssssssssss", $academia, $profesor, $dias, $horario, $apartado, $nombre, $apellido, $numero_usuario, $email, $paquete, $precio, $folio, $fecha_hora, $reglamento, $firma);
        if ($stmt->execute()) {
